...
安装器只修改明确拥有的文件和标记区块，保留无关的个人配置、工具、凭据和用户规则。目标文件已有用户内容但没有 ownership marker 时不得覆盖。默认不安装后台服务、hook 或 index；卸载时只删除本项目拥有的内容。

## 本机启用

将 `package/local.example.toml` 复制到仓库外的本机路径，填写模型、开场指令、GitHub 和可选工具字段，然后运行 `python scripts/install.py install --local-config <local-file>`。用 `codex --profile v23-primary` 启动 V23 主 profile；它选择本机主模型，并把原生 `/review` 映射到本机 review 模型。V23 executor 与 reviewer 仍作为独立 custom agent 注册。
