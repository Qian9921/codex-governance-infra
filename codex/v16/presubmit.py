...
def _check(name: str, why_red: str, cost: str, payload: Mapping[str, Any], *, elapsed: float, exit_status: int) -> dict[str, Any]:
    count_payload = {k: payload[k] for k in ("total", "ran", "passed", "failed", "skipped", "xfail", "unknown") if k in payload}
    counts = validate_counts(count_payload, require_green=False)
...
def render_evidence_comment(envelope: Mapping[str, Any]) -> str:
    """Render the one sanitized immutable PR body from the machine envelope."""
    checks = envelope.get("checks", [])
    lines = [
        "### V16 generated evidence envelope",
        f"- status: `{envelope.get('status')}`",
        f"- exact head: `{envelope.get('head_sha')}`; tree: `{envelope.get('tree_sha')}`",
        f"- envelope SHA-256: `{envelope.get('envelope_sha256')}`",
        "",
        "| check | denominator | passed | failed | skipped | unknown | elapsed |",
    ]
    lines.append("|---|---:|---:|---:|---:|---:|---:|")
    for check in checks:
    ...
    packet = envelope.get("review_packet", {})
    metrics = envelope.get("metrics_dashboard", {}).get("observed", {})
    lines.extend([
        "",
        f"- renderer packet: `{packet.get('schema')}`; author `Qian9921`; reviewer `Liang9921`; coverage `{packet.get('coverage_status')}`; renderer verdict `{packet.get('verdict')}`.",
        f"- observed Spark audits: `{metrics.get('spark_audit_count')}`; gate elapsed: `{metrics.get('gate_elapsed_sec')}s`.",
        "- This body is generated by the local V16 presubmit tool and contains sanitized metadata only.",
    ])
    body = "\n".join(lines) + "\n"
    violations = privacy_scan(body)
    if violations:
        raise RuntimeError("public comment privacy red: " + ",".join(violations))
    return body
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.comment_output:
        destination = pathlib.Path(args.comment_output)
        destination.write_text(render_evidence_comment(result), encoding="utf-8")
