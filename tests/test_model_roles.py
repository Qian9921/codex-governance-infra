...
class ModelRolePolicyTests(unittest.TestCase):
    ...
    def test_author_lineage_sol_consultant_cannot_be_final_reviewer(self):
        review = {
            ...
            "contract_inspection": True,
            "tests_inspection": True,
            "source_derived_counterexample": True,
        }
        with self.assertRaises(ModelRoleError):
            validate_final_review(review, risk="R2", author_lineage=["sol-consultant-vjp"])
    ...
    def test_r2_review_requires_source_derived_counterexample(self):
        review = {
            ...
            "contract_inspection": True,
            "tests_inspection": True,
            "source_derived_counterexample": False,
        }
        with self.assertRaises(ModelRoleError):
            validate_final_review(review, risk="R2")
    ...
    def test_delta_only_reuses_same_reviewer_and_is_limited_to_two_rounds(self):
        ...
        self.assertEqual(validate_final_review(review, risk="R2")["round"], 2)
        review["round"] = 3
        with self.assertRaises(ModelRoleError):
            validate_final_review(review, risk="R2")
